...
use crate::types::{SkipApiResponse, TransferRequest};
...
impl SkipApiClient {
    ...
    pub async fn get_messages(&self, request: &TransferRequest) -> Result<SkipApiResponse> {
        ...
        let messages_request = MessagesRequest {
            ...
            address_list: vec![request.source_address.clone(), request.destination.clone()],
        };
        ...
        info!(
            "Successfully retrieved {} operations from Skip API",
    ...
    pub async fn get_route(&self, request: &TransferRequest) -> Result<SkipApiResponse> {
        ...
        info!(
            "Successfully retrieved route with {} operations",
    ...
    pub fn validate_route(&self, response: &SkipApiResponse) -> Result<()> {
        ...
        if calculated_hash != self.params.expected_route_hash {
            warn!(
                "Route hash mismatch: expected {}, got {}",
                self.params.expected_route_hash, calculated_hash
            );
        ...
        if total_fees > self.params.fee_threshold_token_wei {
            warn!(
                ...
                total_fees, self.params.fee_threshold_token_wei
            );
...
mod tests {
    ...
    async fn test_skip_api_client_creation() {
        ...
        let client_with_key =
            SkipApiClient::new("https://api.skip.build", Some("test-key"), &params).unwrap();

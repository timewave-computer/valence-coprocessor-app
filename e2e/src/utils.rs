...
pub fn validate_cosmos_address(address: &str) -> Result<()> {
    ...
    if address.len() < 39 || address.len() > 63 {
        return Err(anyhow::anyhow!("Invalid Cosmos address length: {}, must be between 39-63 characters", address.len()));
    }
...
pub fn validate_fee_amount(fee_amount: u64, transfer_amount: u64) -> Result<()> {
    ...
    if percentage > 10.0 {
        return Err(anyhow::anyhow!(
            "Fee {}% is too high (max 10%)", 
            percentage
        ));
    }
    
    info!("Fee validation passed: {} wei ({:.4}%)", fee_amount, percentage);
    Ok(())
}
...
pub fn validate_skip_response_structure(response: &serde_json::Value) -> Result<()> {
    ...
    let operations = obj["operations"].as_array()
    ...
    let fees = obj["estimated_fees"].as_array()
    ...
    for operation in operations {
        if operation["type"].as_str() == Some("eureka_transfer") {
            let components = RouteComponents {
                ...
                from_chain_id: operation["from_chain_id"].as_str().unwrap_or("").to_string(),
                to_chain_id: operation["to_chain_id"].as_str().unwrap_or("").to_string(),
                bridge_id: operation["bridge_id"].as_str().unwrap_or("").to_string(),
                entry_contract: operation["entry_contract_address"].as_str().unwrap_or("").to_string(),
...
impl RouteComponents {
    ...
    pub fn validate_against_expected(&self) -> Result<()> {
        if self.operation_type != "eureka_transfer" {
            return Err(anyhow::anyhow!("Expected eureka_transfer, got {}", self.operation_type));
        }
        
        if self.from_chain_id != EXPECTED_SOURCE_CHAIN {
            return Err(anyhow::anyhow!("Expected source chain {}, got {}", EXPECTED_SOURCE_CHAIN, self.from_chain_id));
        }
        
        if self.to_chain_id != EXPECTED_DEST_CHAIN {
            return Err(anyhow::anyhow!("Expected dest chain {}, got {}", EXPECTED_DEST_CHAIN, self.to_chain_id));
        }
        
        if self.bridge_id != EXPECTED_BRIDGE_ID {
            return Err(anyhow::anyhow!("Expected bridge {}, got {}", EXPECTED_BRIDGE_ID, self.bridge_id));
        }
        
        if self.entry_contract.to_lowercase() != EXPECTED_ENTRY_CONTRACT.to_lowercase() {
            return Err(anyhow::anyhow!("Expected entry contract {}, got {}", EXPECTED_ENTRY_CONTRACT, self.entry_contract));
        }
    ...
    pub fn validate_performance(&self) -> Result<()> {
        if let Some(duration) = self.skip_api_route_duration {
            if duration > Duration::from_secs(MAX_API_RESPONSE_TIME_SECONDS) {
                return Err(anyhow::anyhow!("Skip API route call too slow: {:?}", duration));
            }
        }
        
        if let Some(duration) = self.skip_api_messages_duration {
            if duration > Duration::from_secs(MAX_API_RESPONSE_TIME_SECONDS) {
                return Err(anyhow::anyhow!("Skip API messages call too slow: {:?}", duration));
            }
        }
        
        if let Some(duration) = self.ethereum_rpc_duration {
            if duration > Duration::from_secs(MAX_API_RESPONSE_TIME_SECONDS) {
                return Err(anyhow::anyhow!("Ethereum RPC call too slow: {:?}", duration));
            }
        }
...
mod tests {
    ...
    fn test_calculate_fee_percentage() {
        assert_eq!(calculate_fee_percentage(1000000000000000, 10000000000000), 1.0);
